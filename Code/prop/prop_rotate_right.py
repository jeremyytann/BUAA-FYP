...
class PropRotateRightThread(QThread):
  ...
  def run(self):
    ...
    if len(selected_props) > 0:
      ...
      with open("prop_rotate_right_script.py", "w") as f:
        f.write(script_code_formatted)

      try:
        subprocess.run(["python", "prop_rotate_right_script.py"])
      except KeyboardInterrupt:
        pass
      finally:
        os.remove("prop_rotate_right_script.py")